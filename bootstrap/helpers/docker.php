...
function getCurrentApplicationContainerStatus(Server $server, int $id, ?int $pullRequestId = null, ?bool $includePullrequests = false): Collection
{
    $containers = collect([]);
    if (! $server->isSwarm()) {
        $containers = instant_remote_process(["docker ps -a --filter='label=bedrock.applicationId={$id}' --format '{{json .}}' "], $server);
        $containers = format_docker_command_output_to_json($containers);

        $containers = $containers->map(function ($container) use ($pullRequestId, $includePullrequests) {
            $labels = data_get($container, 'Labels');
            $containerName = data_get($container, 'Names');
            $hasPrLabel = str($labels)->contains('bedrock.pullRequestId=');
            $prLabelValue = null;

            if ($hasPrLabel) {
                preg_match('/bedrock\.pullRequestId=(\d+)/', $labels, $matches);
            ...
            if ($pullRequestId !== null && $pullRequestId !== 0 && str($labels)->contains("bedrock.pullRequestId={$pullRequestId}")) {
                return $container;
            }
...
function getCurrentServiceContainerStatus(Server $server, int $id): Collection
{
    $containers = collect([]);
    if (! $server->isSwarm()) {
        $containers = instant_remote_process(["docker ps -a --filter='label=bedrock.serviceId={$id}' --format '{{json .}}' "], $server);
        $containers = format_docker_command_output_to_json($containers);
...
function defaultDatabaseLabels($database)
{
    ...
    $labels->push('bedrock.type=database');
    $labels->push('bedrock.databaseId='.$database->id);
    $labels->push('bedrock.resourceName='.Str::slug($database->name));
    $labels->push('bedrock.serviceName='.Str::slug($database->name));
    $labels->push('bedrock.projectName='.Str::slug($database->project()->name));
    $labels->push('bedrock.environmentName='.Str::slug($database->environment->name));
    $labels->push('bedrock.database.subType='.$database->type());
...
function defaultLabels($id, $name, string $projectName, string $resourceName, string $environment, $pull_request_id = 0, string $type = 'application', $subType = null, $subId = null, $subName = null)
{
    $labels = collect([]);
    $labels->push('bedrock.managed=true');
    $labels->push('bedrock.version='.config('constants.bedrock.version'));
    $labels->push('bedrock.'.$type.'Id='.$id);
    $labels->push("bedrock.type=$type");
    $labels->push('bedrock.name='.Str::slug($name));
    $labels->push('bedrock.resourceName='.Str::slug($resourceName));
    $labels->push('bedrock.projectName='.Str::slug($projectName));
    $labels->push('bedrock.serviceName='.Str::slug($subName ?? $resourceName));
    $labels->push('bedrock.environmentName='.Str::slug($environment));

    $labels->push('bedrock.pullRequestId='.$pull_request_id);
    if ($type === 'service') {
        $subId && $labels->push('bedrock.service.subId='.$subId);
        $subType && $labels->push('bedrock.service.subType='.$subType);
        $subName && $labels->push('bedrock.service.subName='.Str::slug($subName));
    }
...
function fqdnLabelsForTraefik(string $uuid, Collection $domains, bool $is_force_https_enabled = false, $onlyPort = null, ?Collection $serviceLabels = null, ?bool $is_gzip_enabled = true, ?bool $is_stripprefix_enabled = true, ?string $service_name = null, bool $generate_unique_uuid = false, ?string $image = null, string $redirect_direction = 'both', bool $is_http_basic_auth_enabled = false, ?string $http_basic_auth_username = null, ?string $http_basic_auth_password = null)
{
    ...
    if ($serviceLabels) {
        $middlewares_from_labels = $serviceLabels->map(function ($item) {
            ...
            if (preg_match('/bedrock\.traefik\.middlewares=(.*)/', $item, $matches)) {
...
/**
 * Remove Bedrock's custom Docker Compose fields from parsed YAML array
 *
 * Bedrock extends Docker Compose with custom fields that are processed during
 ...
 */
function stripBedrockCustomFields(array $yamlCompose): array
{
...
function validateComposeFile(string $compose, int $server_id): string|Throwable
{
    ...
    try {
        ...
        // Remove Bedrock's custom fields before Docker validation
        $yaml_compose = stripBedrockCustomFields($yaml_compose);
