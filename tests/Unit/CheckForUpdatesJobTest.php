...
it('uses max of CDN and cache versions', function () {
    ...
    File::shouldReceive('get')
        ->with(base_path('versions.json'))
        ->andReturn(json_encode(['bedrock' => ['v4' => ['version' => '4.0.10']]]));
    ...
    File::shouldReceive('put')
        ...
        ->with(base_path('versions.json'), Mockery::on(function ($json) {
            ...
            return $data['bedrock']['v4']['version'] === '4.0.10';
        }));

    Cache::shouldReceive('forget')->once();

    config(['constants.bedrock.version' => '4.0.5']);
...
it('never downgrades from current running version', function () {
    ...
    File::shouldReceive('get')
        ->with(base_path('versions.json'))
        ->andReturn(json_encode(['bedrock' => ['v4' => ['version' => '4.0.5']]]));
    ...
    config(['constants.bedrock.version' => '4.0.10']);
...
it('preserves other component versions when preventing Bedrock downgrade', function () {
    // CDN has older Bedrock but newer Traefik
    Http::fake([
        '*' => Http::response([
            'bedrock' => ['v4' => ['version' => '4.0.0']],
            'traefik' => ['v3.6' => '3.6.2'],
    ...
    File::shouldReceive('put')
        ...
        ->with(base_path('versions.json'), Mockery::on(function ($json) {
            $data = json_decode($json, true);
            // Bedrock should use running version
            expect($data['bedrock']['v4']['version'])->toBe('4.0.10');
    ...
    \Illuminate\Support\Facades\Log::shouldReceive('warning')
        ->once()
        ->with('CDN served older Bedrock version than cache', Mockery::type('array'));
