...
it('ensures stop proxy includes wait loop for container removal', function () {
    ...
    $commands = [
        'docker stop -t 30 bedrock-proxy 2>/dev/null || true',
    ...
    expect($commandsString)->toContain('docker stop -t 30 bedrock-proxy')
        ->and($commandsString)->toContain('docker rm -f bedrock-proxy')
        ->and($commandsString)->toContain('for i in {1..10}; do')
        ->and($commandsString)->toContain('if ! docker ps -a --format "{{.Names}}" | grep -q "^bedrock-proxy$"')
...
it('uses configurable timeout for docker stop', function () {
    ...
    $timeout = 30;
    $stopCommand = "docker stop -t $timeout bedrock-proxy 2>/dev/null || true";
...
it('waits for swarm service container removal correctly', function () {
    ...
    $containerName = 'bedrock-proxy_traefik';
    $checkCommand = "    if ! docker ps -a --format \"{{.Names}}\" | grep -q \"^$containerName$\"; then";

    expect($checkCommand)->toContain('bedrock-proxy_traefik');
});
