...
it('has UpdateBedrock action class', function () {
    expect(class_exists(UpdateBedrock::class))->toBeTrue();
});
...
it('validates cache against running version before fallback', function () {
    ...
    Cache::shouldReceive('remember')
        ->andReturn(['bedrock' => ['v4' => ['version' => '4.0.5']]]);

    config(['constants.bedrock.version' => '4.0.10']);

    $action = new UpdateBedrock;
...
it('uses validated cache when CDN fails and cache is newer', function () {
    ...
    Cache::shouldReceive('remember')
        ->andReturn(['bedrock' => ['v4' => ['version' => '4.0.10']]]);

    config(['constants.bedrock.version' => '4.0.5']);

    // Mock the update method to prevent actual update
    $action = Mockery::mock(UpdateBedrock::class)->makePartial();
