...
it('generates url using base_url instead of APP_URL', function () {
    ...
    ($this->setInstanceSettings)('https://bedrock.example.com');
    ...
    expect($notification->serverUrl)->toBe('https://bedrock.example.com/server/test-server-uuid/security/patches');
});
...
it('includes server url in all notification channels', function () {
    ($this->setInstanceSettings)('https://bedrock.test');
    ...
    expect($discord->description)->toContain('https://bedrock.test/server/abc-123/security/patches');

    // Check Telegram
    $telegram = $notification->toTelegram();
    expect($telegram['buttons'][0]['url'])->toBe('https://bedrock.test/server/abc-123/security/patches');

    // Check Pushover
    $pushover = $notification->toPushover();
    expect($pushover->buttons[0]['url'])->toBe('https://bedrock.test/server/abc-123/security/patches');

    // Check Slack
    $slack = $notification->toSlack();
    expect($slack->description)->toContain('https://bedrock.test/server/abc-123/security/patches');

    // Check Webhook
    $webhook = $notification->toWebhook();
    expect($webhook['url'])->toBe('https://bedrock.test/server/abc-123/security/patches');
});

it('uses correct url in error notifications', function () {
    ($this->setInstanceSettings)('https://bedrock.production.com');
    ...
    $discord = $notification->toDiscord();
    expect($discord->description)->toContain('https://bedrock.production.com/server/error-server-uuid/security/patches');

    // Check error webhook
    $webhook = $notification->toWebhook();
    expect($webhook['url'])->toBe('https://bedrock.production.com/server/error-server-uuid/security/patches')
