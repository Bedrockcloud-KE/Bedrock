...
it('copies the realtime terminal utilities into the container image', function () {
    $dockerfile = file_get_contents(base_path('docker/bedrock-realtime/Dockerfile'));

    expect($dockerfile)->toContain('COPY docker/bedrock-realtime/terminal-utils.js /terminal/terminal-utils.js');
});
...
it('mounts the realtime terminal utilities in local development compose files', function (string $composeFile) {
    ...
    expect($composeContents)->toContain('./docker/bedrock-realtime/terminal-utils.js:/terminal/terminal-utils.js');
})->with([
...
it('keeps realtime terminal server logging restricted to development environments', function () {
    $terminalServer = file_get_contents(base_path('docker/bedrock-realtime/terminal-server.js'));

    expect($terminalServer)
        ->toContain("const terminalDebugEnabled = ['local', 'development'].includes(")
        ->toContain('if (!terminalDebugEnabled) {')
        ->not->toContain("console.log('Bedrock realtime terminal server listening on port 6002. Let the hacking begin!');");
});
