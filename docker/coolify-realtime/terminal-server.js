...
const verifyClient = async (info, callback) => {
    ...
    try {
        ...
        const response = await axios.post(`http://bedrock:8080/terminal/auth`, null, {
            headers: {
...
wss.on('connection', async (ws, req) => {
    ...
    try {
        const response = await axios.post(`http://bedrock:8080/terminal/auth/ips`, null, {
            headers: {
                'Cookie': `${sessionCookieName}=${laravelSession}`,
                'X-XSRF-TOKEN': xsrfToken
            },
...
async function handleCommand(ws, command, userId) {
    ...
    //         Establishes an SSH connection to root@bedrock with RequestTTY enabled
