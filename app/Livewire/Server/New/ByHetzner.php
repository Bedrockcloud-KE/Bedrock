...
class ByHetzner extends Component
{
    ...
    private function createHetznerServer(string $token): array
    {
        ...
        // Prepare SSH keys array: Bedrock key + user-selected Hetzner keys
        $sshKeys = array_merge(
            [$sshKeyId], // Bedrock key (always included)
            $this->selectedHetznerSshKeyIds // User-selected Hetzner keys
        );

        // Remove duplicates in case the Bedrock key was also selected
    ...
    public function submit()
    {
        ...
        try {
            ...
            // Create server in Bedrock database
