...
class CleanupHelperContainersJob implements ShouldBeEncrypted, ShouldBeUnique, ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            $containers = instant_remote_process_with_timeout(['docker container ps --format \'{{json .}}\' | jq -s \'map(select(.Image | contains("'.config('constants.bedrock.registry_url').'/coollabsio/bedrock-helper")))\''], $this->server, false);
