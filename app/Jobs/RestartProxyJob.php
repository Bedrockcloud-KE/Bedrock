...
class RestartProxyJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    private function buildRestartCommands(): array
    {
        ...
        $containerName = $this->server->isSwarm() ? 'bedrock-proxy_traefik' : 'bedrock-proxy';
        ...
        if ($this->server->isSwarmManager()) {
            $commands = $commands->merge([
                ...
                'docker stack deploy --detach=true -c docker-compose.yml bedrock-proxy',
                "echo 'Successfully started bedrock-proxy.'",
            ]);
        } else {
            if (isDev() && $proxyType === ProxyTypes::CADDY->value) {
                $proxy_path = '/data/bedrock/proxy/caddy';
            }
