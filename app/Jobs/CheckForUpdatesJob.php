...
class CheckForUpdatesJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            $response = Http::retry(3, 1000)->get(config('constants.bedrock.versions_url'));
            if ($response->successful()) {
                $versions = $response->json();

                $latest_version = data_get($versions, 'bedrock.v4.version');
                $current_version = config('constants.bedrock.version');

                // Read existing cached version
                $existingVersions = null;
                $existingBedrockVersion = null;
                if (File::exists(base_path('versions.json'))) {
                    $existingVersions = json_decode(File::get(base_path('versions.json')), true);
                    $existingBedrockVersion = data_get($existingVersions, 'bedrock.v4.version');
                }
                ...
                if ($existingBedrockVersion && version_compare($existingBedrockVersion, $bestVersion, '>')) {
                    Log::warning('CDN served older Bedrock version than cache', [
                        'cdn_version' => $latest_version,
                        'cached_version' => $existingBedrockVersion,
                        'current_version' => $current_version,
                    ]);
                    $bestVersion = $existingBedrockVersion;
                }
                ...
                data_set($versions, 'bedrock.v4.version', $bestVersion);
                ...
                // Only mark new version available if Bedrock version actually increased
