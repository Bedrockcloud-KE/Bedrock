...
class InstallDocker
{
    ...
    public function handle(Server $server)
    {
        ...
        if (! $supported_os_type) {
            throw new \Exception('Server OS type is not supported for automated installation. Please install Docker manually before continuing: <a target="_blank" class="underline" href="https://bedrock.io/docs/installation#manually">documentation</a>.');
        }
        ...
        if (isDev() && $server->id === 0) {
            ...
            $command = $command->merge([
                ...
                "echo '{$config}' | base64 -d | tee /etc/docker/daemon.json.bedrock > /dev/null",
                'jq . /etc/docker/daemon.json.bedrock | tee /etc/docker/daemon.json.bedrock.pretty > /dev/null',
                'mv /etc/docker/daemon.json.bedrock.pretty /etc/docker/daemon.json.bedrock',
                "jq -s '.[0] * .[1]' /etc/docker/daemon.json.bedrock /etc/docker/daemon.json | tee /etc/docker/daemon.json.appended > /dev/null",
            ...
            if ($server->isSwarm()) {
                $command = $command->merge([
                    'docker network create --attachable --driver overlay bedrock-overlay >/dev/null 2>&1 || true',
                ]);
            } else {
                $command = $command->merge([
                    'docker network create --attachable bedrock >/dev/null 2>&1 || true',
                ]);
                $command = $command->merge([
                    "echo 'Done!'",
                ]);
