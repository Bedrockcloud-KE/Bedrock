...
class CleanupDocker
{
    ...
    {
        $realtimeImage = config('constants.bedrock.realtime_image');
        $realtimeImageVersion = config('constants.bedrock.realtime_version');
        $realtimeImageWithVersion = "$realtimeImage:$realtimeImageVersion";
        $realtimeImageWithoutPrefix = 'coollabsio/bedrock-realtime';
        $realtimeImageWithoutPrefixVersion = "coollabsio/bedrock-realtime:$realtimeImageVersion";

        $helperImageVersion = getHelperVersion();
        $helperImage = config('constants.bedrock.helper_image');
        $helperImageWithVersion = "$helperImage:$helperImageVersion";
        $helperImageWithoutPrefix = 'coollabsio/bedrock-helper';
        $helperImageWithoutPrefixVersion = "coollabsio/bedrock-helper:$helperImageVersion";
        ...
        // Build image prune command that excludes application images and current Bedrock infrastructure images
        // This ensures we clean up non-Bedrock images while preserving rollback images and current helper/realtime images
        ...
        $commands = [
            'docker container prune -f --filter "label=bedrock.managed=true" --filter "label!=bedrock.proxy=true" --filter "label!=bedrock.type=database" --filter "label!=bedrock.type=application" --filter "label!=bedrock.type=service"',
            $imagePruneCmd,
    ...
    private function buildImagePruneCommand(
        ...
        // Build grep pattern to exclude Bedrock infrastructure images (current version only)
        // This pattern matches the image name regardless of registry prefix:
        // - ghcr.io/coollabsio/bedrock-helper:1.0.12
        // - docker.io/coollabsio/bedrock-helper:1.0.12
        // - coollabsio/bedrock-helper:1.0.12
        // Pattern: (^|/)coollabsio/bedrock-(helper|realtime):VERSION$
        $escapedHelperVersion = preg_replace('/([.\\\\+*?\[\]^$(){}|])/', '\\\\$1', $helperImageVersion);
        $escapedRealtimeVersion = preg_replace('/([.\\\\+*?\[\]^$(){}|])/', '\\\\$1', $realtimeImageVersion);
        $infraExcludePattern = "(^|/)coollabsio/bedrock-helper:{$escapedHelperVersion}$|(^|/)coollabsio/bedrock-realtime:{$escapedRealtimeVersion}$";

        // Delete unused images that:
        // - Are not application images (don't match app repos)
        // - Are not current Bedrock infrastructure images (any registry)
        // - Don't have bedrock.managed=true label
        ...
        $commands[] = "docker images --format '{{.Repository}}:{{.Tag}}' | ".
            $grepCommands.' | '.
            "xargs -r -I {} sh -c 'docker inspect --format \"{{{{index .Config.Labels \\\"bedrock.managed\\\"}}}}\" \"{}\" 2>/dev/null | grep -q true || docker rmi \"{}\" 2>/dev/null' || true";
