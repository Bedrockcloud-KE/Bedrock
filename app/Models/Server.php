...
class Server extends BaseModel
{
    ...
    protected $appends = ['is_bedrock_host'];
    ...
    protected function isBedrockHost(): Attribute
    {
    ...
    {
        $banner =
            "# This file is generated by Bedrock, do not edit it manually.\n".
        ...
        if (isDev()) {
            if ($proxy_type === ProxyTypes::CADDY->value) {
                $dynamic_conf_path = '/data/bedrock/proxy/caddy/dynamic';
            }
        }
   ...
   redir $redirect_url
    ...
    public function setupDynamicProxyConfiguration()
    {
        ...
        if ($this->proxyType() === ProxyTypes::TRAEFIK->value) {
            $file = "$dynamic_config_path/bedrock.yaml";
            if (empty($settings->fqdn) || (isCloud() && $this->id !== 0) || ! $this->isLocalhost()) {
                        ...
                        'services' => [
                            ...
                            'bedrock-realtime' => [
                                'loadBalancer' => [
                                    'servers' => [
                                        0 => [
                                            'url' => 'http://bedrock-realtime:6001',
                                        ],
                                    ],
                                ],
                            ],
                            'bedrock-terminal' => [
                                'loadBalancer' => [
                                    'servers' => [
                                        0 => [
                                            'url' => 'http://bedrock-realtime:6002',
                                        ],
                                    ],
                                ],
                            ],
                ...
                if ($schema === 'https') {
                    $traefik_dynamic_conf['http']['routers']['bedrock-http']['middlewares'] = [
                        0 => 'redirect-to-https',
                    ];

                    $traefik_dynamic_conf['http']['routers']['bedrock-https'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'bedrock',
                    ...
                    $traefik_dynamic_conf['http']['routers']['bedrock-realtime-wss'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'bedrock-realtime',
                    ...
                    $traefik_dynamic_conf['http']['routers']['bedrock-terminal-wss'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'bedrock-terminal',
                ...
                $yaml =
                    "# This file is automatically generated by Bedrock.\n".
            ...
            $file = "$dynamic_config_path/bedrock.caddy";
            if (empty($settings->fqdn) || (isCloud() && $this->id !== 0) || ! $this->isLocalhost()) {
                instant_remote_process([
                    "rm -f $file",
                ], $this);
...
$schema://$host {
    handle /app/* {
        reverse_proxy bedrock-realtime:6001
    }
    handle /terminal/ws {
        reverse_proxy bedrock-realtime:6002
    }
    reverse_proxy bedrock:8080
}";
    ...
    public function reloadCaddy()
    {
        return instant_remote_process([
            'docker exec bedrock-proxy caddy reload --config /config/caddy/Caddyfile.autosave',
        ], $this);
    }

    public function proxyPath()
    {
        $base_path = config('constants.bedrock.base_config_path');
    ...
    public function loadUnmanagedContainers(): Collection
    {
        if ($this->isFunctional()) {
            ...
            $containers = $containers->map(function ($container) {
                ...
                if (! str($labels)->contains('bedrock.managed')) {
                    return $container;
                }
    ...
    public function databases()
    {
        ...
        return $postgresqls
            ...
            ->concat($dragonflies)
            ->concat($clickhouses)
            ->filter(fn ($item) => data_get($item, 'name') !== 'bedrock-db');
    }
    ...
    public function validateDockerEngine($throwError = false)
    {
        ...
        $this->settings->save();
        $this->validateBedrockNetwork(isSwarm: false, isBuildServer: $this->settings->is_build_server);
    ...
    public function validateDockerSwarm()
    {
        ...
        $this->settings->is_usable = true;
        $this->settings->save();
        $this->validateBedrockNetwork(isSwarm: true);
    ...
    public function validateBedrockNetwork($isSwarm = false, $isBuildServer = false)
    {
        if ($isBuildServer) {
            return;
        }
        if ($isSwarm) {
            return instant_remote_process(['docker network create --attachable --driver overlay bedrock-overlay >/dev/null 2>&1 || true'], $this, false);
        } else {
            return instant_remote_process(['docker network create bedrock --attachable >/dev/null 2>&1 || true'], $this, false);
        }
    }
