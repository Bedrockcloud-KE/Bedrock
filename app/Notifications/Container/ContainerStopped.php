...
class ContainerStopped extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        $mail->subject("Bedrock: A resource  has been stopped unexpectedly on {$this->server->name}");
    ...
    {
        $message = "Bedrock: A resource ($this->name) has been stopped unexpectedly on {$this->server->name}";
        $payload = [
            'message' => $message,
        ];
        if ($this->url) {
            $payload['buttons'] = [
                [
                    [
                        'text' => 'Open Application in Bedrock',
                        'url' => $this->url,
                    ],
                ],
            ];
