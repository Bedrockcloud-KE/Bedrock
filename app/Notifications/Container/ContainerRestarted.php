...
class ContainerRestarted extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        $mail->subject("Bedrock: A resource ({$this->name}) has been restarted automatically on {$this->server->name}");
    ...
    {
        $message = "Bedrock: A resource ({$this->name}) has been restarted automatically on {$this->server->name}";
        $payload = [
            'message' => $message,
        ];
        if ($this->url) {
