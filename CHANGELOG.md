...
- Add traefik acme json to bedrock container
- Database secrets
- New servers view
...
- Container logs
- Reset root password
- Attach Bedrock defined networks to services
...
- Add manual update option to UpdateBedrock handle method
...
- Bedrock init should cleanup stuck networks in proxy
- Add manual update check functionality to settings page
- Update auto update and update check frequencies in settings
- Update Upgrade component to check for latest version of Bedrock
...
- Add bedrock build env variables to building phase
...
- Make bedrock full width by default
...
- Add immich service
- Auto generate url
- Refactored to work with bedrock auto env vars
...
- Add upgrade logs to /data/bedrock/source
...
- :passport_control: integrate Authentik authentication with Bedrock
...
- Add is_bedrock_host to the server api responses
...
- *(core)* Wip version of bedrock.json
...
- *(ssl)* Add a Bedrock CA Certificate to all servers
- *(seeder)* Call CA SSL seeder in prod and dev
- *(ssl)* Add Bedrock CA Certificate when adding a new server
...
- *(assets)* Add new SVG logo for Bedrock
...
- *(svg)* Add bedrock and evolution-api SVG logos
...
- *(rules)* Add comprehensive documentation for Bedrock architecture and development practices for AI tools, especially for cursor
...
- *(deployments)* Add support for Bedrock variables in Dockerfile
...
- *(docker-compose)* Add image specifications for bedrock, soketi, and testing-host services
...
- Do not remove bedrock proxy
- Update version
- Be sure .env exists
- Missing fqdn for services
- Default npm command
- Add bedrock-image label for build images
...
- Ignore bedrock proxy error for now
- Python no wsgi
- If user not found
...
- Register should happen if bedrock proxy cannot be started
- GitLab typo
...
- Restart local docker bedrock proxy in case of something happens to it
...
- Check domain for bedrock before saving
- Remove debug info
- Cancel jobs
...
- TrustProxy
- Force restart proxy
- Only restart bedrock proxy in case of version prior to 2.9.2
- Force restart proxy on seeding
- Add GIT ENV variable for submodules
- Recurisve clone instead of submodule
- Versions
- Only reconfigure bedrock proxy if its missconfigured
- Demo version forms
- Typo
...
- Only remove bedrock managed containers
- White-labeled env
- Schema
- Bedrock-network on verification
...
- Dashboard ui
- Create bedrock-infra, if it does not exists
...
- Ui + subscription
- Self-hosted
- Make bedrock-db backups unique dir
...
- Bedrock already exists should not throw error
...
- Encrypt jobs
- ProcessWithEnv()->run
- Plus boarding step about Bedrock
...
- Sentry issue
- Dockercompose save ./ volumes under /data/bedrock
...
- Button title
- Restore falsely deleted bedrock-db-backup
- Sub
...
- Compose env has SERVICE, but not defined for Bedrock
...
- Check proxy functionality before removing unnecessary bedrock.yaml file and checking Docker Engine
...
- Remove network if it is only connected to bedrock proxy itself
...
- Pull bedrock image only when the app needs to be updated
...
- Move mc command to bedrock image from helper
...
- Proxy status
- Bedrock-db should not be in the managed resources
...
- Able to support more database dynamically from Bedrock's UI
- Strapi template
...
- Helper push
- Format
- Add port metadata and Bedrock magic to generate the domain
- Sentinel
- Metrics
...
- Do not stop sentinel on Bedrock restart
...
- Update packagejson of bedrock-realtime + add lock file
...
- Workflow files for bedrock build
- Autocompletes
...
- Use nixpacks predefined build variables, but still could update the default values from Bedrock
...
- *(config)* Update default PostgreSQL host to bedrock-db instead of postgres
...
- *(versions)* Update bedrock version numbers in versions.json and constants.php to 4.0.0-beta.420.5 and 4.0.0-beta.420.6
...
- *(versions)* Update bedrock version numbers in versions.json and constants.php to 4.0.0-beta.420.6 and 4.0.0-beta.420.7
...
- *(constants)* Update bedrock version to 4.0.0-beta.420.7
...
- *(docker-compose)* Set pull_policy to 'never' for bedrock, soketi, and testing-host services
...
- *(versions)* Update bedrock version to 4.0.0-beta.444 and nightly to 4.0.0-beta.445
...
- *(versions)* Update bedrock version to 4.0.0-beta.445 and nightly to 4.0.0-beta.446
...
- Update bedrock version to 4.0.0-beta.446 and nightly version to 4.0.0-beta.447
...
- Update bedrock version to 4.0.0-beta.447 and nightly version to 4.0.0-beta.448
...
- Update bedrock version numbers to 4.0.0-beta.450 and 4.0.0-beta.451
...
- *(ui)* Images inside bedrock changelog (#7357)
...
- *(version)* Update bedrock version to 4.0.0-beta.464 and nightly version to 4.0.0-beta.465
...
- *(fix)* Traefik
- Css
- Error message https://github.com/coollabsio/bedrock/issues/502
- Changes
- Settings
- For removing app
- Local ssh port
...
- Fix
- Fix
- Bedrock proxy access logs exposed in dev
...
- Persist DBs, services and so on stored in data/bedrock
...
- Https://github.com/coollabsio/bedrock/issues/4186
...
- Bump Bedrock to 4.0.0-beta.398
- Bump Bedrock to 4.0.0-beta.400
...
- Preserve clean docker_compose_raw without Bedrock additions
...
- Update version numbers for Bedrock and nightly releases
...
- Prevent bedrock-helper and bedrock-realtime images from being pruned
- Prevent bedrock infrastructure images from being pruned (#7586)
...
- Remove unnecessary logging statements from UpdateBedrock
...
- Remove unused server timezone seeder and related code
- Remove unused PullBedrockImageJob from schedule
...
- Remove unnecessary code for creating bedrock network in upgrade.sh
...
- Update install.sh script to check if bedrock-db volume exists before generating SSH key
...
- Add localhost as Server if it doesn't exist and not in cloud environment
- Update ProductionSeeder to fix issue with bedrock_key assignment
...
- Rename `bedrock.environment` to `bedrock.environmentName`
...
- *(network)* Check for existing bedrock network before creation
...
- *(versions)* Update version numbers for bedrock and nightly
...
- *(ApplicationDeploymentJob)* Streamline environment variable handling by introducing generate_bedrock_env_variables method and consolidating logic for pull request and main branch scenarios
...
- Update changelog
- Update changelog
- *(claude)* Clarify that artisan commands should only be run inside the "bedrock" container during development
...
- Update changelog
- Update changelog
- *(testing-patterns)* Add important note to always run tests inside the `bedrock` container for clarity
- Update changelog
- Update changelog
- Update changelog
...
- Add Bedrock design system reference
- Add Bedrock design system reference (#8237)
- Update changelog
- Update changelog
- Update changelog
...
- Add comment explaining the purpose of disconnecting the network in cleanup_unused_network_from_bedrock_proxy()
...
- Update UpdateBedrockJob timeout to 10 minutes
- Update UpdateBedrockJob to dispatch CheckForUpdatesJob synchronously
...
- Update Bedrock version to 4.0.0-beta.324 and fix file paths in upgrade script
- Update Bedrock version to 4.0.0-beta.324
- Update Bedrock version to 4.0.0-beta.325
- Update Bedrock version to 4.0.0-beta.326
- Add cd command to change directory before removing .env file
- Update Bedrock version to 4.0.0-beta.327
- Update Bedrock version to 4.0.0-beta.328
...
- Update bedrock-helper.yml to get version from versions.json
...
- Update Ray configuration and Dockerfile
- Update bedrock version to 4.0.0-beta.331
...
- Add bedrock.managed=true label to Docker image builds
...
- Update bedrock-helper.yml to include "next" branch in push trigger
...
- Update bedrock environment variable assignment with double quotes
...
- Update Dockerfile and workflow for Bedrock Realtime (v4)
...
- Update Bedrock Realtime workflow to only trigger on the main branch
- Refactor instanceSettings() function to improve code readability
- Update Bedrock Realtime image to version 1.0.2
...
- Bump bedrock-helper version to 1.0.2
...
- Update Docker build and push actions to v6
- Update Docker build and push actions to v6
- Sync bedrock-helper to dockerhub as well
- Push realtime to dockerhub
- Sync bedrock-realtime to dockerhub
- Rename workflows
- Rename development to staging build
- Sync bedrock-testing-host to dockerhbu
- Sync bedrock prod image to dockerhub as well
...
- *(versions)* Update bedrock versions to v4.0.0-beta.388
- *(versions)* Update bedrock versions to v4.0.0-beta.389 and add helper version retrieval script
- *(versions)* Update bedrock versions to v4.0.0-beta.389
- *(core)* EnvironmentVariable Model now extends BaseModel to remove duplicated code
- *(versions)* Update bedrock versions to v4.0.0-beta.3909
- *(version)* Bump Bedrock version to 4.0.0-beta.391
...
- Bump Bedrock version to 4.0.0-beta.392/393
...
- Rollback Bedrock version to 4.0.0-beta.392
- Bump Bedrock version to 4.0.0-beta.393
- Bump Bedrock version to 4.0.0-beta.394
- Bump Bedrock version to 4.0.0-beta.395
- Bump Bedrock version to 4.0.0-beta.396
...
- *(ssl)* Rename CA cert to `bedrock-ca.crt` because of conflicts
...
- *(core)* Remove redocs
- *(versions)* Update bedrock version numbers to 4.0.0-beta.403 and 4.0.0-beta.404
...
- *(versions)* Bump version to 406
- *(versions)* Bump version to 407 and 408 for bedrock and nightly
- *(versions)* Bump version to 408 for bedrock and 409 for nightly
...
- *(versions)* Bump bedrock version to 4.0.0-beta.409 in configuration files
- *(versions)* Bump bedrock version to 4.0.0-beta.410 and update nightly version to 4.0.0-beta.411 in configuration files
- *(templates)* Update plausible and clickhouse images to latest versions and remove mail service
- *(versions)* Update bedrock version to 4.0.0-beta.411 and nightly version to 4.0.0-beta.412 in configuration files
- *(versions)* Update bedrock version to 4.0.0-beta.412 and nightly version to 4.0.0-beta.413 in configuration files
- *(versions)* Update bedrock version to 4.0.0-beta.413 and nightly version to 4.0.0-beta.414 in configuration files
...
- *(versions)* Update bedrock version to 4.0.0-beta.414 and nightly version to 4.0.0-beta.415 in configuration files
- *(workflows)* Adjust workflow for announcement
- *(versions)* Update bedrock version to 4.0.0-beta.416 and nightly version to 4.0.0-beta.417 in configuration files; fix links in deployment view
- *(seeder)* Update git branch from 'main' to 'v4.x' for multiple examples in ApplicationSeeder
- *(versions)* Update bedrock version to 4.0.0-beta.417 and nightly version to 4.0.0-beta.418
- *(versions)* Update bedrock version to 4.0.0-beta.418
- *(versions)* Update bedrock version to 4.0.0-beta.419 and nightly version to 4.0.0-beta.420 in configuration files
...
- *(version)* Update bedrock-realtime to version 1.0.9 in docker-compose and versions files
- *(version)* Update bedrock version to 4.0.0-beta.420 and nightly version to 4.0.0-beta.421
- *(service)* Changedetection remove unused code
- *(service)* Update Evolution API image to the official one (#6031)
- *(versions)* Bump bedrock versions to v4.0.0-beta.420 and v4.0.0-beta.421
...
- *(versions)* Update Bedrock version to 4.0.0-beta.420.1 and add new services (karakeep, miniflux, pingvinshare) to service templates
- *(versions)* Update Bedrock versions to 4.0.0-beta.420.2 and 4.0.0-beta.420.3 in multiple files
- *(versions)* Bump bedrock and nightly versions to 4.0.0-beta.420.3 and 4.0.0-beta.420.4 respectively
- *(versions)* Update bedrock and nightly versions to 4.0.0-beta.420.4 and 4.0.0-beta.420.5 respectively
- *(bump)* Update composer deps
- *(version)* Bump Bedrock version to 4.0.0-beta.420.6
...
- *(versions)* Update bedrock version to 4.0.0-beta.420.8 and nightly version to 4.0.0-beta.420.9 in versions.json and constants.php
- *(docker)* Update soketi image version to 1.0.10 in production and Windows configurations
- *(core)* Update version
- *(core)* Update version
- *(versions)* Update bedrock version to 4.0.0-beta.421 and nightly version to 4.0.0-beta.422
- Update version
- Update development node version
- Update bedrock version to 4.0.0-beta.423 and nightly version to 4.0.0-beta.424
- Update bedrock version to 4.0.0-beta.424 and nightly version to 4.0.0-beta.425
- Update bedrock version to 4.0.0-beta.425 and nightly version to 4.0.0-beta.426
- Update bedrock version to 4.0.0-beta.426 and nightly version to 4.0.0-beta.427
- Update bedrock version to 4.0.0-beta.427 and nightly version to 4.0.0-beta.428
...
- *(versions)* Increment bedrock version to 4.0.0-beta.428 and update realtime_version to 1.0.10
- *(docker)* Add a blank line for improved readability in Dockerfile
- *(versions)* Bump bedrock version to 4.0.0-beta.429 and nightly version to 4.0.0-beta.430
...
- *(versions)* Update bedrock version numbers to 4.0.0-beta.430 and 4.0.0-beta.431 in configuration files
- *(versions)* Increment bedrock version numbers to 4.0.0-beta.431 and 4.0.0-beta.432 in configuration files
- *(versions)* Update bedrock version numbers to 4.0.0-beta.432 and 4.0.0-beta.433 in configuration files
- Remove unused files
- Adjust wording
...
- *(versions)* Update bedrock version to 4.0.0-beta.433 and nightly version to 4.0.0-beta.434 in configuration files
- *(versions)* Update version numbers for Bedrock releases
- *(versions)* Bump Bedrock stable version to 4.0.0-beta.434
- *(versions)* Update Bedrock version numbers to 4.0.0-beta.435 and 4.0.0-beta.436
...
- Update bedrock version to 4.0.0-beta.442
...
- Prepare for PR
- Prepare for PR
- *(version)* Bump bedrock, realtime, and sentinel versions
- *(realtime)* Upgrade npm dependencies
- *(realtime)* Upgrade bedrock-realtime to 1.0.11
- Prepare for PR
- Prepare for PR
- Prepare for PR
...
- Remove bedrock-examples-1 submodule
- *(versions)* Bump bedrock, sentinel, and traefik versions
